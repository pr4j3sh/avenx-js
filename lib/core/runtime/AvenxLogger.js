...
export class AvenxLogger {
  ...
  * The child logger prepends the specified prefix string to every formatted log message.
  * @param {object} [bindings] - Bindings configuration for the child logger.
  * @param {string} [bindings.prefix] - A prefix string prepended to every formatted log message.
  * @returns {AvenxLogger} A new independent logger instance with inherited configuration and bound prefix.
  * @example
  * const authLogger = logger.child({ prefix: '[AuthBridge]' });
  * authLogger.info('User logged in');
  * // Output: [AuthBridge] [Avenx info] User logged in
  */
  child(bindings = {}){
    const childLogger = new AvenxLogger();
    ...
    childLogger.config.formatter = (level, args) => {
      const formatted = parentFormatter(level, args);
      const prefix = bindings.prefix || '';
